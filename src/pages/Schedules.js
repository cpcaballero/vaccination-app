    ...
    <div className=" p-lg-6 p-lg-offset-3">
      ...
      { 
        doses && doses.map( (dose, index) => {
          ...
          let badge = <></>
          let currentSchedule = schedules.find(schedule => schedule._id === dose.schedule);
          ...
          return (
            <Card className="p-col-12" title={ <>Schedule # {index+1}</> } subTitle={<>Status: {badge}</>}>
              <div className="p-grid">
                <div className="p-col-12 p-lg-6">
                  <p><b>Site: </b> { currentSchedule?.location.locationName }</p>
                  <p><b>Date Time Reserved: </b>{ currentSchedule?.scheduleDate } { currentSchedule?.startTime} - { currentSchedule?.endTime} </p>
                  <p><b>Administered On: </b> {""}</p>
                  
                </div>
                <div className="p-col-12 p-lg-6">
                  <p><b>Vaccine Name: </b> {dose.vaccineName}</p>
                  <p><b>Batch Id: </b> {dose.BatchNUmber} </p>
                  <p><b>Dosage: </b> {""} </p>
                </div>
